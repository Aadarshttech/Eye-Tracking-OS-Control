...
\end{center}
\vspace{0.5cm}
Webcam-based eye tracking provides a revolutionary, hands-free methodology for interacting with operating systems, carrying large potential for augmenting accessibility for individuals with severe motor disabilities. However, implementing reliable gaze estimation using standard commodity webcams presents many technical hurdles, including low spatial resolution, unpredictable ambient lighting conditions, large head movement variability, and the infamous "Midas Touch" problem where natural, unintentional eye movements trigger erroneous system interactions. This project successfully conceptualizes, trains, and deploys a comprehensive, real-time eye-tracking pipeline for complete Operating System (OS) control utilizing standard computing hardware. 
...
\begin{table}[H]
    \centering
    \caption{Comparison of Regression Models on Gaze Prediction Error (Euclidean Pixels)}
    \vspace{0.3cm}
...
\section{Findings}
The primary finding during development was the profound impact of data augmentation combined with deep learning optimizations. By mathematically cloning our training data and injecting synthetic Gaussian noise, we prevented model overfitting while providing the neural network enough diverse data to generalize effectively. Coupling this augmented data with a Cosine-Decaying Learning Rate Scheduler allowed the Tuned MLP to reach an absolute mathematical minimum, dropping our Euclidean error from over 90 pixels to 27.4 pixels.
